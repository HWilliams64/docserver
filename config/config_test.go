...
func TestLoadConfig_Defaults(t *testing.T) {
	...
	// Check JWT secret generation (default case)
	assert.NotEmpty(t, cfg.JwtSecret, "JWT Secret should be generated by default")
	assert.Len(t, cfg.JwtSecret, 64, "Generated JWT Secret should be 64 hex chars (32 bytes)")
	// Check if the default key file was created (best effort)
	_, err = os.Stat(defaultJwtKeyFile)
	assert.NoError(t, err, "Default JWT key file should be created when generated")
...
func TestLoadConfig_EnvVars(t *testing.T) {
	...
	t.Setenv("ADDRESS", "192.168.1.100")
	t.Setenv("PORT", "9000")
	t.Setenv("DB_FILE", "/tmp/test_env.json")
	t.Setenv("SAVE_INTERVAL", "15s")
	t.Setenv("ENABLE_BACKUP", "false")
	t.Setenv("JWT_SECRET_FILE", "/etc/secrets/jwt_env.key")
	t.Setenv("JWT_SECRET", "env_secret_key_longer_than_32_bytes") // Set this too, but file should take precedence if file path is set via env
...
func TestLoadConfig_Flags(t *testing.T) {
	...
	// JWT Secret: File is specified by flag, but doesn't exist. No JWT_SECRET env. Should generate.
	assert.NotEmpty(t, cfg.JwtSecret, "JWT Secret should be generated when flag file doesn't exist and no env var")
	assert.Len(t, cfg.JwtSecret, 64, "Generated JWT Secret should be 64 hex chars")
	// Clean up potential generated key file
	os.Remove(defaultJwtKeyFile)
}
...
func TestLoadConfig_Precedence(t *testing.T) {
	...
	t.Setenv("PORT", "9000") // Set Env var
...
func TestLoadConfig_EnableBackupParsing(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.envValue != nil {
				t.Setenv("ENABLE_BACKUP", *tc.envValue)
			} else {
				os.Unsetenv("ENABLE_BACKUP")
			}
...
func TestLoadConfig_JWTSecretHandling(t *testing.T) {
	...
	t.Run("SecretFromFileFlag", func(t *testing.T) {
		...
		defer cleanup()
		os.Unsetenv("JWT_SECRET_FILE") // Ensure flag takes precedence over potential env var for file path
		os.Unsetenv("JWT_SECRET")      // Ensure env secret is not used
	...
	t.Run("SecretFromFileEnv", func(t *testing.T) {
		...
		cleanup := resetFlagsAndArgs() // No flag
		defer cleanup()
		t.Setenv("JWT_SECRET_FILE", tempFile) // Set file path via env
		os.Unsetenv("JWT_SECRET")             // Ensure env secret is not used
	...
	t.Run("SecretFromEnvVar", func(t *testing.T) {
		...
		cleanup := resetFlagsAndArgs() // No flag
		defer cleanup()
		os.Unsetenv("JWT_SECRET_FILE")        // Ensure no file path is set
		t.Setenv("JWT_SECRET", envSecret)     // Set secret via env var
	...
	t.Run("SecretFromEnvVarFallback", func(t *testing.T) {
		...
		defer cleanup()
		os.Unsetenv("JWT_SECRET_FILE")    // Unset env var for file path
		t.Setenv("JWT_SECRET", envSecret) // Set env secret as fallback
	...
	t.Run("GeneratedSecret", func(t *testing.T) {
		cleanup := resetFlagsAndArgs() // No flag
		defer cleanup()
		os.Unsetenv("JWT_SECRET_FILE") // Ensure no file path
		os.Unsetenv("JWT_SECRET")      // Ensure no env secret
		// Clean up potential generated key file from previous runs/other tests
		os.Remove(defaultJwtKeyFile)

		cfg, err := LoadConfig()
		require.NoError(t, err)
		assert.NotEmpty(t, cfg.JwtSecret, "JWT Secret should be generated")
		assert.Len(t, cfg.JwtSecret, 64, "Generated JWT Secret should be 64 hex chars")
		assert.Empty(t, cfg.JwtSecretFile, "JwtSecretFile path should be empty")

		// Check if default key file was created and contains the secret
		generatedKeyBytes, err := os.ReadFile(defaultJwtKeyFile)
		assert.NoError(t, err, "Failed to read default generated key file")
		assert.Equal(t, cfg.JwtSecret, string(generatedKeyBytes), "Generated key file content mismatch")
		// Clean up generated key file
		os.Remove(defaultJwtKeyFile)
	})
	...
	t.Run("GeneratedSecretFallback", func(t *testing.T) {
		...
		defer cleanup()
		os.Unsetenv("JWT_SECRET_FILE") // Unset env var for file path
		os.Unsetenv("JWT_SECRET")      // Unset env secret
		os.Remove(defaultJwtKeyFile)   // Clean up potential generated key file

		cfg, err := LoadConfig()
		require.NoError(t, err)
		assert.NotEmpty(t, cfg.JwtSecret, "JWT Secret should be generated as fallback")
		assert.Len(t, cfg.JwtSecret, 64, "Generated JWT Secret should be 64 hex chars")
		assert.Equal(t, nonExistentFile, cfg.JwtSecretFile, "JwtSecretFile path should still be set from flag")

		// Check if default key file was created (even though flag specified a different file)
		generatedKeyBytes, err := os.ReadFile(defaultJwtKeyFile)
		assert.NoError(t, err, "Failed to read default generated key file")
		assert.Equal(t, cfg.JwtSecret, string(generatedKeyBytes), "Generated key file content mismatch")
		os.Remove(defaultJwtKeyFile) // Clean up
	})
}
...
func TestLoadConfig_DbFilePathAbsolute(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cleanup := resetFlagsAndArgs(args...)
			defer cleanup()
			os.Unsetenv("DB_FILE") // Isolate flag behaviour
